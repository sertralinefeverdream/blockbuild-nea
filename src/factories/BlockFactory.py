    ...
    def create_block(self, game, world, position, block_id, state_data=None):
        if block_id in self._blocks_data.keys():
            ...
            if block_data["type"] == "generic":
                return GenericBlock(game, world, position, block_data["hardness"], block_id, game.block_spritesheet.parse_sprite(block_data["texture"]), block_data["mine_sfx_id"], block_data["place_and_break_sfx_id"], block_data["footstep_sfx_id"], block_data["loot_drop_id"], block_data["loot_drop_tool_whitelist"])
            else:
